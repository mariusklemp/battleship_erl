...
class CNNGenome(object):
    ...
    def configure_new(self, config):
        ...
        self.layer_config = []
        num_conv_layers = randint(1, config.num_cnn_layer)  # Decide the number of Conv layers
        num_fc_layers = randint(1, config.num_fc_layer)  # Decide the number of FC layers
        ...
        for i in range(num_conv_layers):
            try:
                ...
                new_layer = self.add_random_conv_layer(config, in_channels, current_size)

                # Update in_channels and spatial size for the next layer
                new_current_size = ((current_size + 2 * new_layer['padding'] - new_layer['kernel_size']) // new_layer[
                    'stride']) + 1
                ...
                current_size = new_current_size  # Update the spatial size for the next layer
    ...
    def mutate(self, config):
        ...
        if random() < config.layer_add_prob:
            if len(self.layer_config) == 0 or self.layer_config[-1]['layer_type'] == 'conv':
                current_input_size = config.input_size
                in_channels = config.input_channels
                for layer in self.layer_config:
                    if layer['layer_type'] == 'conv':
                        current_input_size = self.calculate_conv_output_size(current_input_size, layer)
                        in_channels = layer['out_channels']

                new_layer = self.add_random_conv_layer(config, in_channels, current_input_size)
                self.layer_config.append(new_layer)

        # Add a new FC layer only if Conv layers exist
        if random() < config.layer_add_prob and any(layer['layer_type'] == 'conv' for layer in self.layer_config):
            last_conv_layer = next(layer for layer in reversed(self.layer_config) if layer['layer_type'] == 'conv')
            current_input_size = config.input_size
            for layer in self.layer_config:
                if layer['layer_type'] == 'conv':
                    current_input_size = self.calculate_conv_output_size(current_input_size, layer)
        ...
        if random() < config.layer_delete_prob and len(self.layer_config) > 1:
            ...
            conv_count = sum(1 for layer in self.layer_config if layer['layer_type'] == 'conv')
            fc_count = sum(1 for layer in self.layer_config if layer['layer_type'] == 'fc')

            # Check layer type before removing
            layer_to_remove = self.layer_config[-1]  # Assume the last layer is targeted for removal
            if (
                    (layer_to_remove['layer_type'] == 'conv' and conv_count > 1) or
    ...
    def _mutate_conv_layer(self, config, layer, index):
        ...
        if random() < config.conv_params_mutate_prob:
            ...
            valid_conv_params = [
                (kernel_size, stride, padding) for kernel_size, stride, padding in config.valid_conv_params
                if kernel_size <= input_size_next_layer  # Ensure the output size is compatible with the next layer
                   and self._is_valid_conv_param(layer['input_size'], kernel_size, stride, padding)
            ]
            ...
            if valid_conv_params:
                ...
                print(f"[ERROR] No valid convolution parameters found for Conv layer {index + 1}.")
        ...
        if index + 1 < len(self.layer_config) and self.layer_config[index + 1]['layer_type'] == 'conv':
    ...
    def _mutate_fc_layer(self, config, layer, index):
        ...
        if random() < 0.3:
            new_fc_layer_size = randint(config.fc_layer_size_min, config.fc_layer_size_max)
        ...
        for i, layer in enumerate(self.layer_config):
            if layer['layer_type'] == 'conv':
                ...
                if layer['kernel_size'] > current_input_size:
                    layer['kernel_size'] = current_input_size  # Adjust kernel size to fit input size
                ...
                if current_input_size <= 0:
                    layer['kernel_size'] = max(1, layer['kernel_size'] - 1)  # Try reducing the kernel size
                    current_input_size = self.calculate_conv_output_size(current_input_size,
                                                                         layer)  # Recalculate with adjusted kernel size
                ...
                flattened_size = layer['fc_layer_size']  # Update for subsequent FC layers
    ...
    def configure_crossover(self, genome1, genome2, config):
        ...
        conv_layers_1 = [layer for layer in genome1.layer_config if layer['layer_type'] == 'conv']
        conv_layers_2 = [layer for layer in genome2.layer_config if layer['layer_type'] == 'conv']
        fc_layers_1 = [layer for layer in genome1.layer_config if layer['layer_type'] == 'fc']
        fc_layers_2 = [layer for layer in genome2.layer_config if layer['layer_type'] == 'fc']
        ...
        for i, (layer1, layer2) in enumerate(zip(conv_layers_1, conv_layers_2)):
            ...
            selected_layer = selected_layer.copy()  # Avoid modifying the parent layers directly
            ...
            try:
                current_input_size = self.calculate_conv_output_size(current_input_size, selected_layer)
                in_channels = selected_layer['out_channels']  # Update in_channels for the next layer
    ...
    def distance(self, other, config):
        ...
        longer_genome = self if len(self.layer_config) > len(other.layer_config) else other
    ...
    def size(self):
        ...
        for layer in self.layer_config:
            if layer['layer_type'] == 'conv':
                ...
                kernel_size = layer['kernel_size']
                in_channels = layer.get('in_channels', 1)  # You might need to store this
