...
class HuntDownStrategy(Strategy):
    ...
    def find_move(self):
        ...
        sunk = {i for i, square in enumerate(self.search_agent.search[3]) if square == 1}
        hits_not_sunk = [i for i, square in enumerate(self.search_agent.search[1]) if square == 1 and i not in sunk]
            ...
            for hit in hits_not_sunk:
                ...
                for r, c in potential_moves:
                    if 0 <= r < self.search_agent.board_size and 0 <= c < self.search_agent.board_size:
                        ...
                        if self.search_agent.search[0][index] == 0:  # Unknown cell
                            return index
